...
const MatchMaking = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get('https://backend-alpha-dusky-96.vercel.app/api/all-books', {
          headers: {
  ...
  const handleRequestExchange = async (bookId) => {
    try {
      ...
      await axios.post(`https://backend-alpha-dusky-96.vercel.app/api/create-request/${bookId}`, { bookListingId: bookId, requesterId: userId }, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
